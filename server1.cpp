...
void send_broadcast(int sender_fd, const std::string& message, 
                   ...
                   std::vector<pollfd>& pollfds)
{
    ...
    {
        if (other_fd == sender_fd)
            continue; // skip the sender

        // Append message to their send buffer
        other_client._message = "Message from client " + std::to_string(sender_fd) + ": " + message + "\r\n";
        ...
        for (size_t i = 0; i < snapshot.size(); ++i)
        {
            ...
            if (fd == listen_fd && (revents & POLLIN))
            {
                ...
                while (true) {
                    ...
                    send_broadcast(fd, message, clients, fd_to_index, pollfds);
                }
            } 
